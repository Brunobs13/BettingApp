...
import datetime
import csv
from multipledispatch import dispatch # dispatch não é uma biblioteca standard pode instala-la com "pip install multipledispatch"
...
class DataStorageBase:
	def __init__(self, arquivo):
		self._arquivo = None#atributo privado que ira armazenar o nome do arquivo
		self.arquivo = arquivo#atribuicao do nome do arquivo atravs do setter
		#getter para o atributo _arquivo
	@property
	def arquivo(self):
		return self._arquivo
	#setter para o atributo _arquivo com validacao de tipo
	@arquivo.setter
	...
	def salvar(self, aposta):
		raise NotImplementedError("Este método deve ser implementado na subclasse.")#metodo deve ser implementado pelas subclasses
...
class DataStorage(DataStorageBase):
	def __init__(self, arquivo):
		...
		if not (arquivo.endswith('.txt') or arquivo.endswith('.csv')):#verifica a extensao
			raise ValueError("O arquivo deve ter a extensão .txt ou .csv.")
	...
	@dispatch(int, list)#sobrecarga do metodo salvar para tratar um argumento int e um argumento list
	def salvar(self, acertos, numeros_sorteados):
...
class DataStorageCSV(DataStorageBase):
	...
	@dispatch(list)# decorador para sobrecarga de metodos.
	def salvar(self, aposta):#metodosalvar no formato csv
		agora = datetime.datetime.now()#obter horas
		linha = f'Aposta feita em {agora.strftime("%d/%m/%Y às %H:%M")}, Aposta:,{", ".join(map(str, aposta))}\n'
		with open(self.arquivo, 'a') as f:
			f.write(linha)
...
class ApostaReader:
	def __init__(self, data_storage):
		self.data_storage = data_storage
		self.apostas = []  # lista vazia para armazenar apostas
		
	def salvar_aposta(self, aposta):  # metodo para salvar aposta
		self.apostas.append(aposta)  # Acessa as apostas a lista
		self.data_storage.salvar(aposta)  # salva a aposta atual
...
class NumerosSorteadosInterface:
	...
	def conferir(self):
		if self.aposta_feita:  # Usa o getter para acessar aposta_feita
			...
			self.data_storage.salvar(acertos, numeros_sorteados)
			if premio > 0:
